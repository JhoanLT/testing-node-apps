...
test('setListItem returns a 403 error if the list item does not belong to the user', async () => {
  ...
  const fakeItemId = 'FAKE_ITEM_ID'
  const next = buildNext()
  const res = buildRes()
  const listItem = buildListItem({id: fakeItemId, ownerId: 'SOMEONE_ELSE'})
  ...
  expect(res.json.mock.calls[0]).toMatchInlineSnapshot(`
    Array [
      Object {
        "message": "User with id FAKE_USER_ID is not authorized to access the list item FAKE_ITEM_ID",
      },
    ]
`)
  expect(next).not.toHaveBeenCalled()
...
test('createListItem creates and returns a list item', async () => {
  ...
  expect(booksDB.readById).toHaveBeenCalledWith(book.id)
  expect(booksDB.readById).toHaveBeenCalledTimes(1)
  expect(res.json).toHaveBeenCalledWith({listItem: {...createListItem, book}})
  expect(res.json).toHaveBeenCalledTimes(1)
})
